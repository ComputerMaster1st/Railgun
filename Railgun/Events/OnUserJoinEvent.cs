...
namespace Railgun.Events
{
    ...
    public class OnUserJoinEvent
    {
        ...
        public OnUserJoinEvent(DiscordShardedClient client, BotLog botLog, IServiceProvider services)
        {
            ...
            client.UserJoined += (user) => Task.Factory.StartNew(async () => await ExecuteAsync(user));
        }
			...
			using (var scope = _services.CreateScope())
			{
				    ...
				    profile.Inactivity.InactiveRoleId != 0)
				{
					if (profile.Inactivity.Users.Any(u => u.UserId == user.Id))
                        ...
                        profile.Inactivity.Users.Add(new UserActivityContainer(user.Id) { LastActive = DateTime.Now });
				}
                ...
                if ((sMention != null && sMention.DisableMentions) || (userProfile != null && userProfile.Globals.DisableMentions))
                    notification = notification.Replace("<user#disc>", $"{username}#{user.DiscriminatorValue}");
