...
namespace Railgun.Commands.Music
{
    public partial class Music
    {
        ...
        {
            [Alias("active"), BotPerms(ChannelPermission.AttachFiles)]
            public class MusicRootActive : SystemBase
            {
                private readonly PlayerController _playerController;

                public MusicRootActive(PlayerController playerController)
                ...
                public Task ExecuteAsync()
                {
                    ...
                    foreach (var info in _playerController.PlayerContainers)
                    {
                        ...
                        output.AppendFormat("Id : {0} {1} Spawned At : {2} {1} Status : {3}", info.GuildId, SystemUtilities.GetSeparator, info.CreatedAt, player.Status).AppendLine()
                            .AppendFormat("\\--> Latency : {0}ms {1} Playing : {2} {1} Since : {3}", player.Latency, SystemUtilities.GetSeparator, song == null ? "Searching..." : song.Metadata.Id.ToString(), player.SongStartedAt).AppendLine().AppendLine();
                    }
