        ...
        public class MusicShow : SystemBase
        {
            ...
            public async Task ExecuteAsync()
            {
                ...
                foreach (var autoJoinConfig in data.AutoJoinConfigs)
                {
                    ...
                    string tempAutoJoinListeners;

                    if (autoJoinConfig.ListenForUsers.Count < 1)
                        tempAutoJoinListeners = "Triggered By All";
                    else
                    {
                        var temp = new StringBuilder()
                            .AppendFormat("Triggered By ");

                        foreach (var user in autoJoinConfig.ListenForUsers)
                            temp.AppendFormat("<[ {0} ]>", await Context.Guild.GetUserAsync(user));

                        tempAutoJoinListeners = temp.ToString();
                    }

                    autoJoinOutput.AppendFormat("| {0}:{1}:{2} |",
                        vc.Name,
                        npTc == null ? (tc == null ? "[UNKNOWN TEXT CHANNEL]" : tc.Name) : "[USING \"Now Playing\" CHANNEL]",
                        tempAutoJoinListeners);
                }
