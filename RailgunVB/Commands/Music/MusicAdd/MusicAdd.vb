    ...
    Partial Public Class Music
        ...
        Partial Public Class MusicAdd
            ...
            Public Async Function UploadAsync() As Task
                ...
                Try
                    Dim song As Song = await _musicService.DownloadSongFromDiscordAsync(attachment.ProxyUrl, 
                    ...
                    Await response.ModifyAsync(Function(c) c.Content = 
                        $"Installed To Playlist - {Format.Bold(song.Metadata.Name)} || ID : {Format.Bold(song.Id)}")
            ...
            <Command("inqueue")>
            Public Async Function InQueueAsync() As Task
                await ReplyAsync($"There are currently {Format.Bold(_musicService.CurrentQueueLength.ToString())} songs waiting within the global processing queue. {Format.Bold((_musicService.CurrentGuildQueueLength(Context.Guild.Id)).ToString())} of which, are from this server.")
            End Function
